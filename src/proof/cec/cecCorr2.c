/**CFile****************************************************************
  ...
  Synopsis    [Incremental signal correspondence computation for &scorr2.]

  Author      [Xiran Zhao]

  Affiliation [University of Chinese Academy of Sciences (UCAS)]

  Description [Extended from the original cecCorr.c implementation by
  Alan Mishchenko.]
...
***********************************************************************/
int Gia_ManCorr2SpecReal( Gia_Man_t * pNew, Gia_Man_t * p, Gia_Obj_t * pObj, int f, int nPrefix )
{
    if ( Gia_ObjIsAnd(pObj) )
    {
        Gia_ManCorr2SpecReduce_rec( pNew, p, Gia_ObjFanin0(pObj), f, nPrefix );
        Gia_ManCorr2SpecReduce_rec( pNew, p, Gia_ObjFanin1(pObj), f, nPrefix );
    ...
    Gia_ManCorr2SpecReduce_rec( pNew, p, Gia_ObjFanin0(pObj), f-1, nPrefix );
...
***********************************************************************/
void Gia_ManCorr2SpecReduce_rec( Gia_Man_t * pNew, Gia_Man_t * p, Gia_Obj_t * pObj, int f, int nPrefix )
{
    Gia_Obj_t * pRepr;
    int iLitNew;
...
***********************************************************************/
Gia_Man_t * Gia_ManCorr2SpecReduce( Gia_Man_t * p, int nFrames, int fScorr, Vec_Int_t ** pvOutputs, int fRings, Vec_Int_t ** pvOutLits )
{
...
***********************************************************************/
Gia_Man_t * Gia_ManCorr2SpecReduceInit( Gia_Man_t * p, int nFrames, int nPrefix, int fScorr, Vec_Int_t ** pvOutputs, int fRings )
{
    Gia_Man_t * pNew, * pTemp;
    Gia_Obj_t * pObj, * pRepr;
    Vec_Int_t * vXorLits;
...
***********************************************************************/
void Cec_Man2StartSimInfo( Vec_Ptr_t * vInfo, int nFlops )
{
    unsigned * pInfo;
    int k, w, nWords;
...
***********************************************************************/
void Gia_ManCorr2RemapSimInfo( Gia_Man_t * p, Vec_Ptr_t * vInfo )
{
    Gia_Obj_t * pObj, * pRepr;
...
***********************************************************************/
Vec_Int_t * Gia_ManCorr2CreateRemapping( Gia_Man_t * p )
{
    Vec_Int_t * vPairs;
    Gia_Obj_t * pObj, * pRepr;
    int i;
...
***********************************************************************/
void Gia_ManCorr2PerformRemapping( Vec_Int_t * vPairs, Vec_Ptr_t * vInfo )
{
    unsigned * pInfoObj, * pInfoRepr;
...
*************************************`**********************************/
int Cec_Man2LoadCounterExamplesTry( Vec_Ptr_t * vInfo, Vec_Ptr_t * vPres, int iBit, int * pLits, int nLits )
{
...
***********************************************************************/
int Cec_Man2LoadCounterExamples( Vec_Ptr_t * vInfo, Vec_Int_t * vCexStore, int iStart )
{
    Vec_Int_t * vPat;
    Vec_Ptr_t * vPres;
    ...
    while ( iStart < Vec_IntSize(vCexStore) )
    {
        ...
        for ( k = 1; k < nBits; k++ )
            if ( Cec_Man2LoadCounterExamplesTry( vInfo, vPres, k, (int *)Vec_IntArray(vPat), Vec_IntSize(vPat) ) )
                break;
...
***********************************************************************/
int Cec_Man2LoadCounterExamplesMapped( Vec_Ptr_t * vInfo, Vec_Int_t * vCexStore, int iStart, Vec_Int_t * vOutBits )
{
    Vec_Int_t * vPat;
    Vec_Ptr_t * vPres;
    int nWords = Vec_PtrReadWordsSimInfo(vInfo);
...
***********************************************************************/
int Cec_Man2LoadCounterExamples2( Vec_Ptr_t * vInfo, Vec_Int_t * vCexStore, int iStart )
{
    unsigned * pInfo;
...
***********************************************************************/
int Cec_Man2ResimulateCounterExamples( Cec_ManSim_t * pSim, Vec_Int_t * vCexStore, int nFrames, Cec_SeedSim_t * pSeed, Vec_Int_t * vOutputs )
{
...
//    pSim->pPars->nWords  = 63;
    ...
    vPairs = Gia_ManCorr2CreateRemapping( pSim->pAig );
    while ( iStart < Vec_IntSize(vCexStore) )
    {
        if ( pSeed )
        {
        ...
        {
            Cec_Man2StartSimInfo( vSimInfo, Gia_ManRegNum(pSim->pAig) );
            iStart = Cec_Man2LoadCounterExamples( vSimInfo, vCexStore, iStart );
            Gia_ManCorr2PerformRemapping( vPairs, vSimInfo );
        }
...
***********************************************************************/
int Cec_Man2ResimulateCounterExamplesComb( Cec_ManSim_t * pSim, Vec_Int_t * vCexStore )
{
    Vec_Ptr_t * vSimInfo;
    ...
    while ( iStart < Vec_IntSize(vCexStore) )
    {
        Cec_Man2StartSimInfo( vSimInfo, 0 );
        iStart = Cec_Man2LoadCounterExamples( vSimInfo, vCexStore, iStart );
        RetValue |= Cec_ManSeqResimulate( pSim, vSimInfo );
    }
    assert( iStart == Vec_IntSize(vCexStore) );
...
/**Function*************************************************************
  ...
  disproval but carries no CEX literals, so Cec_Man2ResimulateCounterExamples
  ...
  refine, matching the established behaviour in Gia_ManCheckRefinements2
...
***********************************************************************/
int Gia_ManCheckRefinements2( Gia_Man_t * p, Vec_Str_t * vStatus, Vec_Int_t * vOutputs, Cec_ManSim_t * pSim, int fRings )
{
    ...
    Vec_StrForEachEntry( vStatus, status, i )
    {
        ...
        if ( status == 0 )
        {
            ...
            // Cec_Man2ResimulateCounterExamples replays the CEX and refines the
...
//                Abc_Print( 1, "Gia_ManCheckRefinements2(): Failed equivalence is not marked as failed!\n" );
...
//    if ( Counter )
//    Abc_Print( 1, "Gia_ManCheckRefinements2(): Could not refine %d nodes.\n", Counter );
    return Counter;
}
...
***********************************************************************/
void Gia_ManCorr2Reduce_rec( Gia_Man_t * pNew, Gia_Man_t * p, Gia_Obj_t * pObj )
{
    Gia_Obj_t * pRepr;
    if ( (pRepr = Gia_ObjReprObj(p, Gia_ObjId(p, pObj))) )
    {
        Gia_ManCorr2Reduce_rec( pNew, p, pRepr );
    ...
    Gia_ManCorr2Reduce_rec( pNew, p, Gia_ObjFanin0(pObj) );
    Gia_ManCorr2Reduce_rec( pNew, p, Gia_ObjFanin1(pObj) );
...
***********************************************************************/
Gia_Man_t * Gia_ManCorr2Reduce( Gia_Man_t * p )
{
    Gia_Man_t * pNew;
    Gia_Obj_t * pObj;
    int i;
...
int Cec_Man2CountLits( Gia_Man_t * p )
{
...
***********************************************************************/
void Cec_ManLSCorrespondenceBmc2( Gia_Man_t * pAig, Cec_ParCor_t * pPars, int nPrefs )
{
    ...
    for ( i = 0; fChanges && (!pPars->nLimitMax || i < pPars->nLimitMax); i++ )
    {
        ...
        if ( Vec_IntSize(vCexStore) )
        {
            ...
            if ( nCexReal > 0 || !pPars->fSkipFailResim )
            {
                ...
                Cec_Man2ResimulateCounterExamples( pSim, vCexStore, nBmcResimFrames, NULL, vOutputs );
            }
            if ( nCexTriv > 0 )
                Cec_ManTrivialSatSplit( pAig, pSim, vCexStore, vStatus, vOutputs, pPars->fUseRings );
            Gia_ManCheckRefinements2( pAig, vStatus, vOutputs, pSim, pPars->fUseRings );
            fChanges = 1;
        }
        if ( pPars->fVerbose )
            Cec_Man2RefinedClassPrintStats( pAig, vStatus, -1, Abc_Clock() - clkBmc );
        // recycle
...
***********************************************************************/
int Cec_ManLSCorrAnalyzeDependence2( Gia_Man_t * p, Vec_Int_t * vEquivs, Vec_Str_t * vStatus )
{
...
***********************************************************************/
int Cec_ManLSCorrespondenceClasses2( Gia_Man_t * pAig, Cec_ParCor_t * pPars )
{
    int nIterMax     = 100000;
    ...
    if ( fRunBmcFirst && (!pPars->fLatchCorr || pPars->nFrames > 1) )
        Cec_ManLSCorrespondenceBmc2( pAig, pPars, 0 );
    if ( Cec_ParCorShouldStop( pPars ) )
    {
        Cec_ManSimStop( pSim );
        return 1;
...
//        Cec_ManLSCorrAnalyzeDependence2( pAig, vOutputs, vStatus );
        ...
        clk2 = Abc_Clock();
        {
            ...
            if ( nCexReal > 0 || !pPars->fSkipFailResim )
            {
                Cec_Man2ResimulateCounterExamples( pSim,
        ...
        if ( pPars->fVerbose )
            Cec_Man2RefinedClassPrintStats( pAig, vStatus, r+1, Abc_Clock() - clk );
        Vec_StrFree( vStatus );
        Vec_IntFree( vOutputs );
//Gia_ManEquivPrintClasses( pAig, 1, 0 );
        ...
        if ( pPars->nLimitMax )
        {
            int nCur = Cec_Man2CountLits(pAig);
    ...
    if ( pPars->fKissatCert && r < nIterMax )
    {
        ...
        else
        {
            ...
            pCertSrm = Gia_ManCorr2SpecReduce( pAig, pPars->nFrames,
            ...
            if ( !fCertFailed )
            {
                ...
                pCertSrm = Gia_ManCorr2SpecReduceInit( pAig, pPars->nFrames, 0,
    ...
    if ( pPars->fVerbose )
        Cec_Man2RefinedClassPrintStats( pAig, NULL, r+1, Abc_Clock() - clk );
    // check the overflow
    if ( r == nIterMax )
...
***********************************************************************/
unsigned * Cec_Man2ComputeInitState( Gia_Man_t * pAig, int nFrames )
{
...
***********************************************************************/
void Cec_Man2PrintFlopEquivs( Gia_Man_t * p )
{
    Gia_Obj_t * pObj, * pRepr;
    int i;
...
***********************************************************************/
Gia_Man_t * Cec_ManLSCorrespondence2( Gia_Man_t * pAig, Cec_ParCor_t * pPars )
{
    Gia_Man_t * pNew, * pTemp;
    unsigned * pInitState;
    int RetValue;
    ...
    if ( pPars->nPrefix == 0 )
    {
        RetValue = Cec_ManLSCorrespondenceClasses2( pAig, pPars );
        if ( RetValue == 0 )
    ...
    else
    {
        ...
        pInitState = Cec_Man2ComputeInitState( pAig, pPars->nPrefix );
        pTemp = Gia_ManDupFlip( pAig, (int *)pInitState );
        ABC_FREE( pInitState );
        // compute classes of this AIG
        RetValue = Cec_ManLSCorrespondenceClasses2( pTemp, pPars );
        ...
        Cec_ManLSCorrespondenceBmc2( pAig, pPars, pPars->nPrefix );
/*
...
***********************************************************************/
Vec_Wec_t * Gia_ManCreateRegSupps2( Gia_Man_t * p, int fVerbose )
{
...
Vec_Int_t * Gia_ManFindStopFlops2( Gia_Man_t * p, int nFlopIncFreq, int fVerbose )
{
    Vec_Int_t * vRes = NULL, * vTemp;  int i, k, Spot, Temp, nItems = 0;
    Vec_Wec_t * vSupps = Gia_ManCreateRegSupps2( p, fVerbose );
...
Gia_Man_t * Gia_ManDupStopsAdd2( Gia_Man_t * p, Vec_Int_t * vStops )
{
    Gia_Man_t * pNew;
...
void Gia_ManDupStopsRem2_rec( Gia_Man_t * pNew, Gia_Man_t * p, Gia_Obj_t * pObj )
{
    if ( ~pObj->Value )
        return;
    assert( Gia_ObjIsAnd(pObj) );
    Gia_ManDupStopsRem2_rec( pNew, p, Gia_ObjFanin0(pObj) );
    Gia_ManDupStopsRem2_rec( pNew, p, Gia_ObjFanin1(pObj) );
    pObj->Value = Gia_ManAppendAnd( pNew, Gia_ObjFanin0Copy(pObj), Gia_ObjFanin1Copy(pObj) );
}
Gia_Man_t * Gia_ManDupStopsRem2( Gia_Man_t * p, Vec_Int_t * vStops )
{
    Gia_Man_t * pNew;
...
Gia_Man_t * Gia_ManDupStopsTest2( Gia_Man_t * p )
{
    Vec_Int_t * vStops = Gia_ManFindStopFlops2( p, 1, 1 );
    if ( vStops == NULL )
        return Gia_ManDup(p);
    Gia_Man_t * pNew1 = Gia_ManDupStopsAdd2( p, vStops );
    Gia_Man_t * pNew2 = Gia_ManDupStopsRem2( pNew1, vStops );
    Gia_ManStop( pNew1 );
    Vec_IntFree( vStops );
    return pNew2;
}
